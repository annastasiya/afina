...
void ServerImpl::RunAcceptor() {
    ...
    while (running.load()) {
        ...
        // }
        {
            ...
            if (connections.size() < max_workers) { // connections.size() = 99, max_workers=100
                pthread_t client_thread;
                if (pthread_create(&client_thread, nullptr, RunConnectionProxy, this) == 0) {
...
void ServerImpl::RunConnection() {
    ...
    // main loop is over, time to cleanup
    shutdown(client, SHUT_RDWR);
    close(client);

    pthread_t client_thread = pthread_self();
    connections.erase(client_thread);
